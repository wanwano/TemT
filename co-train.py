...
def train(args):
    if "ICEWS" in args.dataset:
        ...
        settings['CI'] = 1
        settings['time_scale'] = 1
        tpred_a, tpred_b, tpred_c = 1, 3, 10  # in years
        loss_scaling_factor = 5e-5

    data_dir = args.data_dir

    use_cuda = torch.cuda.is_available()
    num_nodes, num_rels, num_t = utils.get_total_number(data_dir + args.dataset, 'stat.txt')
    train_data, _ = utils.load_quadruples(data_dir + args.dataset, 'train.txt')
    test_data, _ = utils.load_quadruples(data_dir + args.dataset, 'test.txt')
    test_filtering_file_objs = '/test_subcentric_filtering_objs.txt'
    test_filtering_file_subs = '/test_objcentric_filtering_subs.txt'

    with open(data_dir + args.dataset + test_filtering_file_objs, 'r') as f:
    ...
    with open(data_dir + args.dataset + test_filtering_file_subs, 'r') as f:
    ...
    with open(data_dir + args.dataset + train_sub_tp, 'rb') as f:
        s_history_event_tp = pickle.load(f)
    with open(data_dir + args.dataset + train_ob_tp, 'rb') as f:
        o_history_event_tp = pickle.load(f)
    with open(data_dir + args.dataset + train_sub_dt_tp, 'rb') as f:
        s_history_dt_tp = pickle.load(f)
    with open(data_dir + args.dataset + train_ob_dt_tp, 'rb') as f:
        o_history_dt_tp = pickle.load(f)
    with open(data_dir + args.dataset + train_dur, 'rb') as f:
        dur_last_data = pickle.load(f)
    with open(data_dir + args.dataset + train_sub_synchro_dt, 'rb') as f:
        sub_synchro_dt_data = pickle.load(f)
    with open(data_dir + args.dataset + train_obj_synchro_dt, 'rb') as f:
        obj_synchro_dt_data = pickle.load(f)
    with open(data_dir + args.dataset + train_sub_lk, 'rb') as f:
        s_history_event_lk = pickle.load(f)
    with open(data_dir + args.dataset + train_ob_lk, 'rb') as f:
        o_history_event_lk = pickle.load(f)
    with open(data_dir + args.dataset + train_sub_dt_lk, 'rb') as f:
        s_history_dt_lk = pickle.load(f)
    with open(data_dir + args.dataset + train_ob_dt_lk, 'rb') as f:
        o_history_dt_lk = pickle.load(f)

    with open(data_dir + args.dataset + test_sub_tp, 'rb') as f:
        s_history_test_event_tp = pickle.load(f)
    with open(data_dir + args.dataset + test_ob_tp, 'rb') as f:
        o_history_test_event_tp = pickle.load(f)
    with open(data_dir + args.dataset + test_sub_dt_tp, 'rb') as f:
        s_history_test_dt_tp = pickle.load(f)
    with open(data_dir + args.dataset + test_ob_dt_tp, 'rb') as f:
        o_history_test_dt_tp = pickle.load(f)
    with open(data_dir + args.dataset + test_dur, 'rb') as f:
        dur_last_test = pickle.load(f)
    with open(data_dir + args.dataset + test_sub_synchro_dt, 'rb') as f:
        sub_synchro_dt_test = pickle.load(f)
    with open(data_dir + args.dataset + test_obj_synchro_dt, 'rb') as f:
        obj_synchro_dt_test = pickle.load(f)
    with open(data_dir + args.dataset + test_sub_lk, 'rb') as f:
        s_history_test_event_lk = pickle.load(f)
    with open(data_dir + args.dataset + test_ob_lk, 'rb') as f:
        o_history_test_event_lk = pickle.load(f)
    with open(data_dir + args.dataset + test_sub_dt_lk, 'rb') as f:
        s_history_test_dt_lk = pickle.load(f)
    with open(data_dir + args.dataset + test_ob_dt_lk, 'rb') as f:
        o_history_test_dt_lk = pickle.load(f)

    with open(data_dir + args.dataset + '/train_graphs.txt', 'rb') as f:
        train_graph_dict = pickle.load(f)

    with open(data_dir + args.dataset + '/test_graphs.txt', 'rb') as f:
        test_graph_dict = pickle.load(f)

    model = TemT(args, num_nodes, num_rels)
    ...
    if not args.only_eva:
        ...
        while True:
            ...
            for batch_data in utils.make_batch(train_data, s_history_event_tp,
                ...
                cro_entr_lk, error_tp, _, _, _, _, _, _, _, _ = model(batch_data, args.timepre_mode, 'Training', train_graph_dict)
    ...
    for batch_data in utils.make_batch(test_data, s_history_test_event_tp, s_history_test_event_lk,
        ...
        _batch += 1
        # 这个模式下，每个计算结果的requires_grad=False
        with torch.no_grad():
            sub_rank, obj_rank, cro_entr_lk, error_tp, density_tp, dt_tp, mae_tp, gt_tp, den1_tp, den2_tp, \
            tpred, abs_error = model(batch_data, args.timepre_mode, 'Test', test_graph_dict)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='TemT')
    parser.add_argument("--dropout", type=float, default=0.5, help="dropout probability")
    parser.add_argument("--n-hidden", type=int, default=200, help="number of hidden units")
    parser.add_argument("--lr", type=float, default=3e-3, help="learning rate")
    parser.add_argument("--max_hist_len", type=int, default=8, help="maximum history sequence length")
    parser.add_argument("--grad-norm", type=float, default=1.0, help="norm to clip gradient to")
    parser.add_argument("--rnn-layers", type=int, default=1)
    parser.add_argument("--path_num", type=int, default=8)
    ...
    parser.add_argument("-d", "--dataset", type=str, default='YAGO', help="dataset to use")
    parser.add_argument("--embd_rank", type=int, default=200)
    parser.add_argument("--max-epochs", type=int, default=15, help="maximum epochs")
    ...
    parser.add_argument("--eva_dir", type = str, help="saved dir of the testing model")
