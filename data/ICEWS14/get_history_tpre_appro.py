...
def get_history(dataset, dataset_name, latest_t):
    ...
    dur_last_t = [ -1 for _ in range(len(dataset))]
    sub_synchro_dt = [ -1 for _ in range(len(dataset))]
    obj_synchro_dt = [ -1 for _ in range(len(dataset))]
    s_history_entities_tpre = reset_hist_lists(num_dataset=len(dataset))
    s_history_times_tpre = reset_hist_lists(num_dataset=len(dataset))
...
if __name__ == '__main__':
    ...
    events_t = dict()
    s_his_idx = [[dict() for _ in range(num_e)] for _ in range(num_r)]
